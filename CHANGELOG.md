...
- 文本对象的映射必须用 `:<C-u>call` 而不是 `<Cmd>`（`<Cmd>` 映射不许换模式），
  于是进 `TextObject()` 时用户已经被带出了可视模式。答得出来时会重新划一个选区，
  但两条答不出来的路径 —— 缓存冷、以及链上没有这一类节点（例如没有类型的文件里
  按 `ac`）—— 都直接 return，把人留在普通模式里。而 `af`/`aa` 这些默认键在普通
  模式下是「append 一个 f」：帮助里那句「再按一次就好了」于是变成一次静默的改
  文件（`let a = 2;` → `let af = 2;`）。原生文本对象在这里是响一声、选区照旧。
- 可视模式的那份映射现在多传一个真值，`TextObject()` 在两条失败路径上都 `gv`
  把选区还回去 —— `SelectExpand()`/`SelectShrink()` 本来就是这么做的。算子等待
  模式不受影响：算子照旧被放弃，buffer 一个字节都不动。
- `tests/vim_smoke.vim` 补上冷缓存与无此类节点两种情形的可视模式回归，连同
  「重按一次」这一步一起走完；文档也说清楚了两种模式各自的表现。

### 修复：折叠开关来回拨一次之后折叠就死了

- `FoldsDigestToSend()` 里那条「折叠层级数组还得跟 buffer 行数一样长」的判断没有
  任何测试覆盖 —— 唯一冲着它去的断言传的是一个从没记过摘要的 bufnr，三行之前就
  在第一个条件上短路了，把这条判断整段删掉 `make check` 照样绿。
- 而它是有用的：`:TsHlFoldsToggle` 关闭时 `RestoreFoldSettings()` 清掉
  `s_fold_exprs` 却留着 `s_folds_digest`，中间不编辑再打开，重新请求就会带着那份
  陈旧摘要，daemon 回 `unchanged`，`ApplyFolds()` 被跳过，于是 `'foldexpr'` 还在、
  `FoldExpr()` 却对每一行都答 `=`，折叠一直死到下一次编辑。
- 断言改成走完整的「关掉再打开、中间不编辑」流程：折叠层级数组要重新铺满整个
  buffer，`FoldExpr(1)` 不能是 `=`。删掉那条判断，这组断言现在会红。
